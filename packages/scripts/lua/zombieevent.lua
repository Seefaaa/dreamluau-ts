...
local makeZombieController = function(location)
	-- Do not use /mob/eye/camera/ai here: it is owned by a real silicon AI
	-- and depends on the station camera network. A plain eye with its own
	-- perspective gives the controller the same free-moving AI-eye behavior.
	...
	-- /mob/eye has no faction list by default.
	controller.faction = { "zombie" }
	...
	-- /mob/eye is not inserted here automatically. Register its starting Z
	-- immediately, then keep the list in sync when it flies between Z levels.
	local oldZ = controller.z
	if oldZ and dead_players_by_zlevel[oldZ] then
		list.add(dead_players_by_zlevel[oldZ], controller)
	end
	...
	["Zombie Controller"] = {
		onGain = function(self, humanData)
			...
			if mind then
				...
				-- Look through the controller itself, just as an AI looks through
				-- its eye, rather than retaining the deleted human's perspective.
